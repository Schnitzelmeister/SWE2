	...
	public void Initialize(Diagram diagram, JPanel container) {
		...
		if (dataArea != null) {
			for (int r = dataArea.getFirstRow() + 1; r <= dataArea.getLastRow(); ++r) {
				...
				ds.addSeries(dataArea.getParent().getCell(r, dataArea.getFirstColumn()).getTextValue(), data);
			}
		}
		// Generate the graph
		chart = ChartFactory.createXYLineChart(diagram.getName(), "x", "y", ds, PlotOrientation.VERTICAL, true, true, false);
...
class BarDiagramInvalidateStrategy implements DiagramInvalidateStrategy {
	...
	public void Initialize(Diagram diagram, JPanel container) {
		...
		if (dataArea != null) {
			for (int r = dataArea.getFirstRow() + 1; r <= dataArea.getLastRow(); ++r) {
				for (int c = dataArea.getFirstColumn() + 1; c <= dataArea.getLastColumn(); ++c) {
					ds.addValue(dataArea.getParent().getCell(r, c).getNumericValue(), dataArea.getParent().getCell(r, dataArea.getFirstColumn()).getTextValue(), dataArea.getParent().getCell(dataArea.getFirstRow(), c).getTextValue());
				}
			}
		}

		// Generate the graph
		chart = ChartFactory.createBarChart(diagram.getName(), "x", "y", ds);
