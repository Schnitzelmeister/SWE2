...
public class Application implements ActionListener, WorkbookListener {
	...
	public void writeCSV(String workSheetName, String filePath) throws IOException {
		...
		// CsvWriteUtility.convertWorkSheetToCsv(Workbook.getSheet(workSheetName),
		// writer); Sobald Csv File ge�ffnet werden kann
		writer.flush();
		writer.close();
	}
		...
		openFileMenuItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					// JOptionPane.showMessageDialog(this, "Die Datei konnte
					// nicht gefunden werden ", "Fehler",
					...
					if (returnVal == JFileChooser.APPROVE_OPTION) {
						...
						try {
							if (chooser.getFileFilter().getDescription().contains("CSV")) {
								writeCSV(tabbedPane.getTitleAt(tabbedPane.getSelectedIndex()),
										choosedAbsolutFile = chooser.getSelectedFile().getAbsolutePath());
							} else if (chooser.getFileFilter().getDescription().contains("PDF")) {
								writePDF(tabbedPane.getTitleAt(tabbedPane.getSelectedIndex()),
										choosedAbsolutFile = chooser.getSelectedFile().getAbsolutePath());
							}
	...
	public void afterCellChanged(String worksheetName, int row, int column, Object newValue) {
		System.out.println(
				"AfterCellChanged '" + worksheetName + "'!R" + row + "C" + column + " = " + newValue.toString());
	}
